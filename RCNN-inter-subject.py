'''
RCNN for inter-subject sleep stage classification
'''
import numpy as np
...
model_file = 'weights/sleepnet-rcnn-inter-subject.hdf5' # the name of the file storing the trained cnn model
...
test_gen = generatorWithShuffle(lookback, min_index=5201, max_index=None, batch_size=batch_size, step=step)
...
#model.load_weights('weights/sleepnet-rcnn-epoch100.hdf5')
model.compile(loss='categorical_crossentropy', optimizer=optimizers.Adam(lr=0.001, decay=0.0001), metrics=['accuracy'])

#filepath = 'weights/sleepnet-' + model.name + '-2layerlstm-epoch100.hdf5'
